...
## 4. Publicarea este automată — `.github/workflows/wiki-sync.yml`

Fișierele din [`wiki/`](../wiki) nu ajung automat pe `github.com/totpero/DeviceDetector.NET/wiki` doar prin faptul că sunt comise în acest repo — wiki-ul e un repo Git separat (`DeviceDetector.NET.wiki.git`). Acest lucru este automatizat prin workflow-ul GitHub Actions [`wiki-sync.yml`](../.github/workflows/wiki-sync.yml):

- **Trigger**: la fiecare `push` pe `master` care modifică ceva sub `wiki/**`, plus manual din tab-ul **Actions** (`workflow_dispatch`).
- **Ce face**:
  1. Face checkout la acest repo (directorul `wiki/`).
