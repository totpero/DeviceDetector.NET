...
{
    /// <summary>
    /// Creates an isolated temp directory that mirrors matomo-icons' dist/ folder layout for a test,
    ...
    internal sealed class TempIconDirectory : IDisposable
    {
        ...
        {
            RootPath = Path.Combine(Path.GetTempPath(), "MatomoIconResolverTests_" + Guid.NewGuid().ToString("N"));
