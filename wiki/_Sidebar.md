**DeviceDetector.NET**

- [Home](Home)
- [Getting Started](Getting-Started)
- [Device & Client Type Checks](Device-and-Client-Type-Checks)
- [Client Hints](Client-Hints)
- [Caching](Caching)
- [Regex Engines](Regex-Engines)
- [Bot Detection](Bot-Detection)
- [ASP.NET Core Integration](ASP.NET-Core-Integration)
- [Icon Packs](Icon-Packs)
- [Serilog Enricher](Serilog.Enrichers.AspNetCore.DeviceDetector)
- [CacheBuilder Tool](CacheBuilder-Tool)
- [Updating Device Detector Data](Updating-Device-Detector-Data)
- [FAQ](FAQ)
- [Contributing](Contributing)

---
[NuGet](https://www.nuget.org/packages/DeviceDetector.NET) · [Gitter](https://gitter.im/totpero/DeviceDetector.NET) · [Upstream PHP project](https://github.com/matomo-org/device-detector)
